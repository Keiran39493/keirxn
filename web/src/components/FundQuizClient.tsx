...
import { FUND_QUIZ_STEPS } from "@/lib/fund-quiz-steps";
import type { FundQuizAnswers, QuizOptionValue } from "@/lib/types";
...
export function FundQuizClient() {
  ...
  const select = (stepId: string, value: QuizOptionValue) => {
    const stepDef = FUND_QUIZ_STEPS.find((s) => s.id === stepId);
    if (!stepDef) return;
  ...
  const buildPayload = (): FundQuizAnswers => ({
    org_role: String(answers.org_role ?? "staff"),
    sectors: (answers.sectors as string[]) ?? [],
    funding_need: (answers.funding_need as string[]) ?? [],
    amount_sought: String(answers.amount_sought ?? "25k_100k"),
  ...
  const submit = async () => {
    setSubmitting(true);
    try {
      const resp = await fetch("/api/match-funds", {
        method: "POST",
      ...
      sessionStorage.setItem(
        STORAGE_KEYS.fundMatchResults,
        JSON.stringify(results),
      );
      router.push("/ngo/results");
    } catch {
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className="list-check flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-amber-600">
                            <svg className="h-3 w-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                            </svg>
                          </span>
        ...
        <div className="mt-10 flex items-center justify-between">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-2 rounded-xl bg-amber-600 px-8 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-amber-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:shadow-none"
          >
            {submitting ? (
              <>
